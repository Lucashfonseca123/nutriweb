...
class CreateItemcardapioTable extends Migration
{
    ...
    public function up()
    {
        Schema::create($this->tableName, function (Blueprint $table) {
            ...
            $table->integer('Refeicao_idRefeicao');
            ...
            $table->foreign('Cardapio_idCardapio', 'fk_ItemCardapio_Cardapio1_idx')
                ->references('idCardapio')->on('Cardapio')
                ->onDelete('no action')
                ->onUpdate('no action');

            $table->foreign('Grupo_idGrupo', 'fk_ItemCardapio_Grupo1_idx')
                ->references('idGrupo')->on('Grupo')
                ->onDelete('no action')
                ->onUpdate('no action');

            $table->foreign('Refeicao_idRefeicao', 'fk_ItemCardapio_Refeicao1_idx')
